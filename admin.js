...
function showAdd_watch() {
    ...
    document.getElementById('cateWatchInput').value = '';
...
function AddWatch() {
    ...
    var cate =  document.getElementById('cateWatchInput').value;
...
function UpdateWatch(id) {
    for (var i = 0; i < watch.length; i++) {
        if (id == watch[i].id) {
            ...
            document.getElementById("cateWatchInput").value = watch[i].cate;
...
function SaveUpdateWatch() {
    ...
    for (var i = 0; i < watch.length; i++) {
        if (idchange == watch[i].id) {
            ...
            watch[i].cate = document.getElementById("cateWatchInput").value;
...
function ManagementWatch() {
    ...
    for (let i = 0; i < watch.length; i++) {
        var prinfManage = `<tr>
  <td><div>
    <div class="cart_img_box float-left bg-warning" style="width: 15%;height: 100px;">
      <img src="` + watch[i].image + `" width="100px" height="100px">
    </div>
    <div class="cart_info_box float-left pl-3" style="width: 70%;height: 80px;">
      <p class="mb-1 font-weight-bold" style="font-size: 115%;">` + watch[i].name + `</p>
      <p style="font-size: 85%">` + watch[i].note + `</p>
    </div>
  </div> </td>
  <td class="text-center"><p class="mt-2" style="padding:5px;">` + watch[i].cate + `</p> </td>
  <td class="text-center"><p class="mt-2" style="padding:5px;">` + watch[i].price + `</p> </td>
  <td class="text-center">'<div class="btn btn-danger text-white ml-2" onclick="DeleteWatch(` + watch[i].id + `)" style="width: 16%;border-radius: 10px;"><i class="fa fa-trash" aria-hidden="true"></i></div>
<div class="btn btn-warning  text-white mr-2" onclick="UpdateWatch(` + watch[i].id + `)" style="width: 16%;border-radius: 10px;"><i class="fa fa-pencil-square-o" aria-hidden="true"></i></div> </td>
</tr>`;
...
function paymentPrinf() {
  ...
  <td>` + paymentWatch[i].showDetailOrder + `<p id="commentPrinfPayment` + paymentWatch[i].id_payment + `"></p></td>
  <td class="text-center">` + paymentWatch[i].priceTotal + `</td>
  <td class="text-center"><button id="status_prinf` + paymentWatch[i].id_payment + `" onclick="verify_payment(` + paymentWatch[i].id_payment + `)" class="btn btn-warning text-white">` + paymentWatch[i].status + `</button></td>
</tr>
`
...
function paymentPrinfDash() {
  ...
  <td>` + paymentWatch[i].showDetailOrder + `</td>
